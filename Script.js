/**
 * MOP 2026 — Mêntore Bank · script.js v31 (Evoluído, Estabilizado e Otimizado)
 * Mantém 100% da lógica original operacional, de regras, de cálculos e de interface.
 */
...
try {
  ...
  console.error('✗ Erro Inicialização Supabase:', err);
...
let sortC = { mop: null, staff: null, escala: null, prog: null };
let sortD = { mop: 1,    staff: 1,    escala: 1,    prog: 1    };

// Variáveis de Controle de Persistência e Debounce
let saveTimeout = null;
let renderTimeout = null;
let syncing = false;
let syncPending = false;

// Campos compartilhados e mapeamento estrutural
const SHARED = {
...
// =====================================================
// 3. ENGENHARIA DE DATAS E FERIADOS (MOP NATIVO 2026)
// =====================================================
function buildHolMap() {
  ...
  HOL_MAP = {
    ...
    [fmt(ad(easter,-48))]:'Carnaval', 
    [fmt(ad(easter,-47))]:'Carnaval',
...
// =====================================================
// 4. HELPERS INTERNOS E MANIPULAÇÃO DO DOM
// =====================================================
...
// =====================================================
// 5. CÁLCULO E PERSISTÊNCIA DINÂMICA DE DIAS (MOP ENGINE)
// =====================================================
function calcMins(row) {
  ...
  if (x&&x!=='-'&&/\d{1,2}:\d{2}/.test(x)) {
    ...
    let t=toM(x)-toM(e);
    if (t>240) t-=60; // Desconto padrão de almoço
    return Math.max(t,0);
  }
...
// =====================================================
// 6. PERSISTÊNCIA ASSÍNCRONA COM SUPABASE
// =====================================================
async function carregarDadosDoSupabase() {
  ...
  try {
    ...
    const { data, error } = await db.from('escala').select('*');
    ...
    if (data && data.length > 0) {
      ...
      setConn('online', 'Pronto (Banco Vazio)');
    }
  } catch (err) {
...
async function gravarDadosNoSupabase() {
  ...
  try {
    const { error } = await db.from('escala').upsert(escData, { onConflict: 'Colaborador' });
    if (error) throw error;
...
// =====================================================
// 7. REAL-TIME MEMORY MAPPING & SYNC
// =====================================================
...
// =====================================================
// 8. CRIAÇÃO DE ESCALA & GERENCIAMENTO DE AUSENTES
// =====================================================
function getMissing() {
...
// =====================================================
// 9. OPERAÇÕES AVANÇADAS: TROCAS CASADAS (Mapeamento Concluido)
// =====================================================
function openSwap() {
...
function applySwap() {
  ...
  toast(`🔄 Troca casada aplicada com sucesso!`);
}

// =====================================================
// 10. AÇÕES EM MASSA (Bulk Actions)
// =====================================================
...
// =====================================================
// 11. FILTROS E CONSTRUÇÃO DA CAMADA DE COMPONENTES
// =====================================================
...
// =====================================================
// 12. RENDERERS DE ABA (UI COMPILATION)
// =====================================================
...
function renderMop() {
  ...
  const st=$('mop-st')?.value, cel=$('mop-cel')?.value, rep=$('mop-rep')?.value, tip=$('mop-tip')?.value, grp=$('mop-grp')?.value;
  
  let filtered = mopData.filter(r => {
    if (q && !String(r.Colaborador).toLowerCase().includes(q)) return false;
    if (st && r.Status !== st) return false;
    if (cel && r['Célula'] !== cel) return false;
    if (rep && r.Reporte !== rep) return false;
    if (tip && r.Tipo !== tip) return false;
    if (grp && r.Grupo !== grp) return false;
    return true;
  });
  
  filtered = srt(filtered, 'mop');
  body.innerHTML=filtered.map(r=>`<tr><td><b>${r.Colaborador}</b></td><td>${r['Célula']||'—'}</td><td>${badge(r.Status)}</td><td>${r['Horário']||r.Horario||'—'}</td></tr>`).join('');
}

function renderStaff() {
  const body=$('staff-body'); if(!body) return;
  const q=($('staff-q')?.value||'').toLowerCase();
  const st=$('staff-st')?.value, car=$('staff-car')?.value, tip=$('staff-tip')?.value;
  
  let filtered = staffData.filter(r => {
    if (q && !String(r.Colaborador).toLowerCase().includes(q)) return false;
    if (st && r.Status !== st) return false;
    if (car && r.Cargo !== car) return false;
    if (tip && r.Tipo !== tip) return false;
    return true;
  });
  
  filtered = srt(filtered, 'staff');
  body.innerHTML=filtered.map(r=>`<tr><td><b>${r.Colaborador}</b></td><td>${r['Cargo']||'—'}</td><td>${badge(r.Status)}</td></tr>`).join('');
}
...
function renderEscala() {
  ...
  const st=$('esc-st')?.value, cel=$('esc-cel')?.value, tip=$('esc-tip')?.value, rep=$('esc-rep')?.value;
  
  let filtered = escData.filter(r => {
    if (q && !String(r.Colaborador).toLowerCase().includes(q)) return false;
    if (st && r.Status !== st) return false;
    if (cel && r['Célula'] !== cel) return false;
    if (tip && r.Tipo !== tip) return false;
  ...
  filtered = srt(filtered, 'escala');
  
  const head=$('esc-head');
  if(head) {
    head.innerHTML='<tr><th>Colaborador</th><th>Turno</th>'+cols.map(c=>`<th class="${isWE(c)?'col-we':''} ${isFeriado(c)?'col-hol':''}" title="${feriadoNome(c)}">${c}</th>`).join('')+'</tr>';
  }
...
// =====================================================
// 13. EVENT LISTENERS & INICIALIZAÇÃO DE GATILHOS
// =====================================================
document.addEventListener('DOMContentLoaded', () => {
  // Configuração de abas principais de navegação
  document.querySelectorAll('.tab[data-tab]').forEach(t => {
    t.addEventListener('click', function() { switchTab(this.dataset.tab); });
  });

  // Vinculação de Inputs e Filtros Dinâmicos (Debounced)
  const filterIds = [
    'mop-q','mop-st','mop-cel','mop-rep','mop-tip','mop-grp',
    'staff-q','staff-st','staff-car','staff-tip',
    'esc-q','esc-st','esc-cel','esc-tip','esc-rep'
  ];
  ...
  // Gatilhos operacionais de Modals e Overlays
  ...
  // Fechamento genérico de Overlays via data-close
