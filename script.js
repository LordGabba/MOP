/**
 * MOP 2026 — Mêntore Bank · script.js v31 (Integrado com Supabase)
 */
...
async function carregarDadosDoSupabase() {
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      // Captura as colunas de datas dinâmicas a partir da estrutura salva
      ALL_DAY_COLS = Object.keys(data[0]).filter(k => k.includes('-') || k.includes('/'));
      
      // Identifica os meses presentes na base para alimentar o filtro temporal
      const ms = new Set();
      ALL_DAY_COLS.forEach(c => { const m = c.match(/(\d{2})[-\/](\d{2})$/); if(m) ms.add(m[2]); });
      MONTHS = [...ms].sort();
      if (!curMonth && MONTHS.length) curMonth = MONTHS[0];

      // Mapeia os dados paralelos para manter a retrocompatibilidade visual
      ...
      // Renderiza a aba atual de forma dinâmica
      switchTab(curTab);
...
// =====================================================
// 8. TROCAS CASADAS E EM MASSA (Ações operacionais)
// =====================================================
function openSwap() {
  const wrap=$('swap-wrap'); if(!wrap) return;
  if (wrap.style.display!=='none') { wrap.style.display='none'; return; }
  const bw=$('bulk-wrap'); if(bw) bw.style.display='none';
  ...
  ['swap-a','swap-b','swap-from','swap-to'].forEach(id=>{
    const el=$(id); if(el) el.onchange=updateSwapPreview;
  });
  updateSwapPreview();
...
function applySwap() {
  ...
  const dcsToSwap = (from&&to) ? ALL_DAY_COLS.filter(c=>{ const d=colToDate(c); return d&&d>=from&&d<=to; }) : ALL_DAY_COLS;
  dcsToSwap.forEach(col=>{ const tmp=rA[col]; rA[col]=rB[col]; rB[col]=tmp; });
  ...
  markUnsaved(); renderEscala();
  const st=$('swap-status'); if(st) { st.textContent='✅ Troca realizada!'; st.style.color='var(--grn)'; }
  toast(`🔄 Troca casada: ${nA.split(' ')[0]} ↔ ${nB.split(' ')[0]}`);
  updateSwapPreview();
}
...
function renderEscala() {
  ...
  // Atualiza dinamicamente os cabeçalhos das datas na tabela
  const head=$('esc-head');
  if(head) {
...
// =====================================================
// 11. EVENT LISTENERS GLOBAIS E INICIALIZAÇÃO DE BOTÕES
// =====================================================
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Gatilhos de modals e overlays de ação
  ...
  const bsa=$('btn-swap-apply'); if(bsa) bsa.addEventListener('click', applySwap);
  
  // Fechamento de Overlays genéricos
  document.querySelectorAll('[data-close]').forEach(btn => {
    btn.addEventListener('click', () => closeOv(btn.dataset.close));
  });

  // Validação de preenchimento de escala individual
