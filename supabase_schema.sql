-- ============================================================
-- SISTEMA DE GESTÃƒO OPERACIONAL - SUPABASE SCHEMA COMPLETO
-- ============================================================
...
-- ============================================================
-- FUNÃ‡ÃƒO: atualiza updated_at automaticamente
-- ============================================================
...
BEGIN
  FOR r IN
    ...
    WHERE schemaname = 'public'
      AND tablename IN ('colaboradores','staff','escalas','programacoes','auditoria','usuarios','configuracoes','usuarios_autorizados','auth_logs')
  LOOP
...
USING (
  app_is_authorized()
  AND (
    app_has_profile(ARRAY['ADMIN','GESTOR','CONSULTA'])
    OR (app_has_profile(ARRAY['AGENTE']) AND LOWER(email) = app_user_email())
  )
);

CREATE POLICY colaboradores_insert_admin_gestor ON colaboradores
FOR INSERT TO authenticated
WITH CHECK (app_has_profile(ARRAY['ADMIN','GESTOR']));

CREATE POLICY colaboradores_update_admin_gestor ON colaboradores
FOR UPDATE TO authenticated
USING (app_has_profile(ARRAY['ADMIN','GESTOR']))
WITH CHECK (app_has_profile(ARRAY['ADMIN','GESTOR']));
...
FOR SELECT TO authenticated
USING (app_has_profile(ARRAY['ADMIN','GESTOR','CONSULTA']));
...
USING (
  app_has_profile(ARRAY['ADMIN','GESTOR','CONSULTA'])
  OR (app_has_profile(ARRAY['AGENTE']) AND colaborador_id = app_colaborador_id())
);
...
WITH CHECK (app_has_profile(ARRAY['ADMIN']));
-- ============================================================
-- DADOS INICIAIS DE CONFIGURAÃ‡ÃƒO
-- ============================================================
INSERT INTO configuracoes (chave, valor, descricao) VALUES
  ('tema', '"claro"', 'Tema da interface'),
  ('empresa', '"Minha Empresa"', 'Nome da empresa'),
  ('celulas', '["CÃ©lula A","CÃ©lula B","CÃ©lula C","Suporte","OperaÃ§Ãµes"]', 'CÃ©lulas disponÃ­veis'),
  ('grupos', '["Grupo 1","Grupo 2","Grupo 3","Especialistas"]', 'Grupos disponÃ­veis'),
  ('horarios', '["06:00-14:00","14:00-22:00","22:00-06:00","08:00-17:00","09:00-18:00","10:00-19:00"]', 'HorÃ¡rios padrÃ£o'),
  ('filiais', '["Matriz","Filial SP","Filial RJ","Filial BH"]', 'Filiais cadastradas'),
  ('tipos_escala', '["Normal","Day Off","FÃ©rias","Folga","Treinamento","Home Office","LicenÃ§a"]', 'Tipos de escala'),
  ('cargos', '["Analista","Coordenador","Gerente","Assistente","Especialista","Supervisor","Diretor"]', 'Cargos disponÃ­veis')
